...
void cRocket::render()
{
		...
		glTranslatef(spritePos2D.x, 768, 0.0f);
...
void cRocket::update(float deltaTime)
{
		...
		if (m_InputMgr->isKeyDown(VK_SPACE))
		{
			...
			theRocketBullets[numBullets]->setSpritePos(glm::vec2(spritePos2D.x + spriteCentre.x, 768));
			...
			theRocketBullets[numBullets]->setTextureDimensions(theGameTextures[4]->getTWidth()*0.8, theGameTextures[4]->getTHeight() / 2);
		...
		*/
		...
		if (astro < 1)
		{
			...
			theAsteroids[astro]->setSpriteTranslation(glm::vec2(0, 20 * counter));
